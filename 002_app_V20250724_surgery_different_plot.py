...
###############################################################################
# CHANGE: Update create_animated_evolution_chart for separate figures
def create_animated_evolution_chart(df_final_dict, clf_model, predictions_df, threshold_values=None):
    if isinstance(df_final_dict, dict):
        ...
        # Create two separate chart placeholders
        st.subheader("Laparoscopic Sleeve Gastrectomy (LSG)")
        lsg_chart_placeholder = st.empty()
        lsg_prob_placeholder = st.empty()
        
        st.subheader("Laparoscopic Roux-en-Y Gastric Bypass (LRYGB)")  
        lrygb_chart_placeholder = st.empty()
        lrygb_prob_placeholder = st.empty()
        ...
        dm_prob_columns = ['DMII_preoperative_prob', 'dm3m_prob', 'dm6m_prob', 'dm12m_prob', 'dm18m_prob', 'dm2y_prob', 'dm3y_prob', 'dm4y_prob', 'dm5y_prob']
        ...
        smooth_lsg_dm_prob = make_interp_spline(time_points_values, lsg_dm_probabilities)(smooth_years)
        smooth_lsg_dm_prob = np.array([min(1, max(0, x)) for x in smooth_lsg_dm_prob])
        ...
        smooth_lrygb_dm_prob = make_interp_spline(time_points_values, lrygb_dm_probabilities)(smooth_years)
        smooth_lrygb_dm_prob = np.array([min(1, max(0, x)) for x in smooth_lrygb_dm_prob])
        ...
        def create_individual_chart(years, bmi_values, ci_upper, ci_lower, bmi_min, bmi_max, surgery_name):
            # BMI data
            bmi_data = pd.DataFrame({
                "Year": years,
                "BMI": bmi_values,
                "Line": [f"{surgery_name} BMI"] * len(years)
            })
            
            # CI data
            ci_data = pd.DataFrame({
                "Year": np.concatenate([years, years]),
                "BMI": np.concatenate([ci_upper, ci_lower]),
                "Line": ["CI"] * len(years) * 2
            })
            
            # Create charts
            bmi_chart = alt.Chart(bmi_data).mark_line(strokeWidth=3).encode(
                ...
                color=alt.Color("Line", 
                    scale=alt.Scale(domain=[f"{surgery_name} BMI", "CI", "Healthy BMI"],
                                  range=["blue", "orange", "green"])),
                tooltip=["Year:Q", "BMI:Q", "Line:N"]
            )
            
            ci_chart = alt.Chart(ci_data).mark_line(strokeDash=[2, 2], strokeWidth=2).encode(
                x="Year",
            ...
            healthy_chart = alt.Chart(healthy_bmi_line[:len(years)]).mark_line(strokeWidth=1.5).encode(
                x="Year",
                y=alt.Y("BMI", scale=alt.Scale(domain=[bmi_min, bmi_max])),
                color=alt.Color("Line", 
                    scale=alt.Scale(domain=[f"{surgery_name} BMI", "CI", "Healthy BMI"],
            ...
            return (ci_chart + healthy_chart + bmi_chart).properties(width=850, height=400)
        
        # Animation loop for both charts simultaneously
        for i in range(1, total_steps + 1, 1):
            current_years = smooth_years[:i]
            
            # Create LSG chart
            lsg_chart = create_individual_chart(
                current_years, smooth_lsg_bmi[:i], smooth_lsg_upper[:i], 
                smooth_lsg_lower[:i], bmi_min, bmi_max, "LSG"
            )
            lsg_chart_placeholder.altair_chart(lsg_chart)
            
            # Create LRYGB chart  
            lrygb_chart = create_individual_chart(
                current_years, smooth_lrygb_bmi[:i], smooth_lrygb_upper[:i],
                smooth_lrygb_lower[:i], bmi_min, bmi_max, "LRYGB"
            )
            lrygb_chart_placeholder.altair_chart(lrygb_chart)
            
            # Update probability displays if patient has preoperative diabetes
            if lsg_data['DMII_preoperative'].values[0] == 1:
                ...
                if current_time_months == 0:
                    ...
                    years_val = current_time_months / 12
                    time_label = f"{years_val:.1f} years"
                
                # LSG probability display
                lsg_current_prob = smooth_lsg_dm_prob[i-1]
                lsg_color = "red" if lsg_current_prob > 0.8 else "orange" if lsg_current_prob > 0.5 else "green"
                
                lsg_prob_placeholder.markdown(f"""
                <div style="text-align: center; padding: 15px; background-color: #f0f2f6; border-radius: 10px; margin: 10px 0;">
                    <p style="font-size: 18px; margin: 5px 0; color: #666;">Time: <strong>{time_label}</strong></p>
                    <p style="font-size: 20px; margin: 5px 0; color: {lsg_color}; font-weight: bold;">
                        DM Probability: {lsg_current_prob:.1%}
                    </p>
                </div>
                """, unsafe_allow_html=True)
                
                # LRYGB probability display
                lrygb_current_prob = smooth_lrygb_dm_prob[i-1]
                lrygb_color = "red" if lrygb_current_prob > 0.8 else "orange" if lrygb_current_prob > 0.5 else "green"
                
                lrygb_prob_placeholder.markdown(f"""
                <div style="text-align: center; padding: 15px; background-color: #f0f2f6; border-radius: 10px; margin: 10px 0;">
                    <p style="font-size: 18px; margin: 5px 0; color: #666;">Time: <strong>{time_label}</strong></p>
                    <p style="font-size: 20px; margin: 5px 0; color: {lrygb_color}; font-weight: bold;">
                        DM Probability: {lrygb_current_prob:.1%}
                    </p>
                </div>
                """, unsafe_allow_html=True)
        ...
        # Cards for the full information - BOTH surgeries
        if lsg_data['DMII_preoperative'].values[0] == 1:
            ...
            # LSG Cards Section
            st.subheader("LSG - Diabetes Risk and BMI at Key Time Points")
            
            # Create columns for LSG
            lsg_cols = st.columns(3)
            
            for idx, (months, time_label, bmi_col, prob_col) in enumerate(time_points_display):
                col_idx = idx % 3
                
                current_bmi = lsg_data[bmi_col].values[0]
                current_dm_probability = lsg_data[prob_col].values[0]
            ...
            # LRYGB Cards Section
            st.subheader("LRYGB - Diabetes Risk and BMI at Key Time Points")
            
            # Create columns for LRYGB
            lrygb_cols = st.columns(3)
            
            for idx, (months, time_label, bmi_col, prob_col) in enumerate(time_points_display):
                col_idx = idx % 3
                
                current_bmi = lrygb_data[bmi_col].values[0]
                current_dm_probability = lrygb_data[prob_col].values[0]
            ...
            # Optional: Add a comparison summary table
            st.subheader("Surgery Comparison Summary")
            
            # Create comparison dataframe
            comparison_data = []
            for months, time_label, bmi_col, prob_col in time_points_display:
                lsg_bmi = lsg_data[bmi_col].values[0]
                lsg_dm_prob = lsg_data[prob_col].values[0]
                lrygb_bmi = lrygb_data[bmi_col].values[0]
                lrygb_dm_prob = lrygb_data[prob_col].values[0]
                
                comparison_data.append({
                    'Time Point': time_label,
                    'LSG BMI': f'{lsg_bmi:.1f}',
                    'LSG DM Risk': f'{lsg_dm_prob:.1%}',
                    'LRYGB BMI': f'{lrygb_bmi:.1f}',
                    'LRYGB DM Risk': f'{lrygb_dm_prob:.1%}',
                    'BMI Difference': f'{lsg_bmi - lrygb_bmi:+.1f}',
                    'DM Risk Difference': f'{lsg_dm_prob - lrygb_dm_prob:+.1%}'
                })
            ...
            #st.markdown("""
            #**Interpretation:**
            #- **BMI Difference**: Positive values mean LSG results in higher BMI than LRYGB
            #- **DM Risk Difference**: Positive values mean LSG has higher diabetes risk than LRYGB
            #- Cards with **blue border** show LSG results, **red border** show LRYGB results
            #""")
